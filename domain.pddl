...
(:action move
    ...
    :precondition (and (not (stopped ?r))
                       (at ?r ?from)
                       (battery ?r ?fpre)
...
(:action recharge
    ...
    :precondition (and (not (= ?rfrom ?rto))
                       (at ?rfrom ?loc)
                       (at ?rto ?loc)
    ...
    :effect (and (not (battery ?rfrom ?fpre-from))
...
(:action stop-and-guard
    ...
    :effect (and (stopped ?r) (guarded ?l)
            (forall (?l2 - location)
...
(:action verify-guard-config
    :parameters (?c - config)
    :precondition (and (forall (?l - location)
                               (imply (GUARD-CONFIG ?c ?l) (guarded ?l))))
    :effect (and (forall (?r - robot) (not (stopped ?r)))
